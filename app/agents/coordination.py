...
class Coordination(File):
    ...
    def __init__(self, tools) -> None:
        ...
        self.team_search = SearchTeam(tools=tools)
    ...
    async def supervisor_general_agent_node(self, state: MessagesState) -> Command[Literal["supervisor_search_agent", "supervisor_action_agent", END]]: # type: ignore

        if isinstance(state["messages"][-1], AIMessage):
            return Command(goto=END) # type: ignore

        response = await self.supervisor_general_chain.ainvoke({"messages": state["messages"]})
