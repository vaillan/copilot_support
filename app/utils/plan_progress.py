...
def construir_ledger(progreso: Dict[str, Any], idioma: str = "es") -> str:
    ...
    texto_ninguno = obtener_mensaje("ledger.pendientes_ninguno", idioma)
    texto_completados = ", ".join(str(n) for n in completados) if completados else texto_ninguno

    if paso_actual > total:
        return (
            f"{obtener_mensaje('ledger.pasos_completos', idioma, lista=texto_completados)} · "
            f"{obtener_mensaje('ledger.todos_completos', idioma, total=total)} · "
            f"{obtener_mensaje('ledger.pendientes', idioma, lista=texto_ninguno)}"
        )

    pendientes = [n for n in range(1, total + 1) if n not in completados and n != paso_actual]
    texto_pendientes = ", ".join(str(n) for n in pendientes) if pendientes else texto_ninguno
    return (
        f"{obtener_mensaje('ledger.pasos_completos', idioma, lista=texto_completados)} · "
        f"{obtener_mensaje('ledger.paso_actual', idioma, paso_actual=paso_actual, total=total)} · "
        f"{obtener_mensaje('ledger.pendientes', idioma, lista=texto_pendientes)}"
    )


def construir_contexto_compacto(plan: Any, progreso: Optional[Dict[str, Any]], idioma: str = "es") -> Optional[str]:
    ...
    ledger = construir_ledger(progreso, idioma)
    total = len(pasos)

    if paso_actual > total:
        return f"{ledger}\n\n{obtener_mensaje('ledger.contexto_todos_completos', idioma)}"
