...
@patch('app.agents.agente_revisor.get_reviewer_llm')
@patch('app.agents.agente_revisor.fileSystem.get_file_content')
def test_agente_revisor_recibe_solo_pasos_con_test(mock_get_file, mock_get_llm, mock_state):
    """
    El revisor recibe en el prompt solo los pasos con requiere_test=True
    (vía construir_plan_pruebas) junto con el reporte del codificador.
    """
    mock_llm = MagicMock()
    mock_get_llm.return_value = mock_llm
    mock_get_file.return_value = "Plan a revisar:\n{plan}\n\nCodigo escrito:\n{codigo_escrito}"

    mock_llm.bind_tools.return_value.invoke.return_value = AIMessage(
        content="",
        tool_calls=[{"name": "finalizar_revision", "args": {"aprobado": True}, "id": "call_rev_ok"}],
    )

    mock_state["plan_de_accion"] = {
        "pasos": [
            {"archivo": "app/core.py", "tarea": "**Paso 1: modulo con test**\nImplementar PASO_CON_TEST con pytest.", "requiere_test": True},
            {"archivo": "README.md", "tarea": "**Paso 2: documentacion**\nActualizar PASO_SIN_TEST en el readme.", "requiere_test": False},
        ]
    }
    mock_state["codigo_escrito"] = "reporte de cambios del codificador"

    result = agente_revisor(mock_state)

    assert result.goto == END
    prompt = mock_llm.bind_tools.return_value.invoke.call_args[0][0]
    contenido = prompt.to_messages()[0].content
    assert "PASO_CON_TEST" in contenido
    assert "reporte de cambios del codificador" in contenido
    assert "PASO_SIN_TEST" not in contenido


@patch('app.agents.agente_planificador.get_planner_llm')
@patch('app.agents.agente_planificador.fileSystem.get_file_content')
def test_agente_planificador_instruccion_en_todas_las_iteraciones(mock_get_file, mock_get_llm, mock_state):
    """
    La instrucción original del usuario se inyecta en el system prompt del
    planificador en todas las iteraciones del bucle de investigación.
    """
    mock_llm = MagicMock()
    mock_get_llm.return_value = mock_llm
    mock_get_file.return_value = "system prompt"
    ...
    mock_llm.bind_tools.return_value.invoke.side_effect = [
        AIMessage(content="", tool_calls=[{"name": "read_file", "args": {"file_path": "main.py"}, "id": "call_read"}]),
        AIMessage(content="", tool_calls=[{"name": "entregar_plan_de_accion", "args": {"explicacion_arquitectura": "plan pagos", "pasos": []}, "id": "call_plan"}]),
    ]

    mock_state["instruccion_usuario"] = "Crea un modulo de pagos con IVA 21"

    resultado1 = agente_planificador(mock_state)
    assert resultado1.goto == "nodo_herramientas_planificador"

    mock_state["messages"] = mock_state["messages"] + (resultado1.update or {}).get("messages", [])

    resultado2 = agente_planificador(mock_state)
    assert resultado2.goto == "agente_codificador"

    llamadas = mock_llm.bind_tools.return_value.invoke.call_args_list
    assert len(llamadas) == 2
    for llamada in llamadas:
        assert "Crea un modulo de pagos con IVA 21" in llamada.args[0].to_messages()[0].content