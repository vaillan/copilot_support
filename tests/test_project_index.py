...
def test_resumir_archivo_go(tmp_path):
    """Test que el resumidor genérico captura imports y firmas de Go."""
    archivo = tmp_path / "main.go"
    archivo.write_text('package main\n\nimport "fmt"\n\nfunc main() { fmt.Println("hola") }\n', encoding="utf-8")
    resumen = resumir_archivo(archivo, max_tokens=400)

    assert "resumen" in resumen
    assert "imports" in resumen
    assert "firmas" in resumen
    assert any(i.startswith('import "fmt"') for i in resumen["imports"])
    assert any(f.startswith("func main") for f in resumen["firmas"])


def test_resumir_archivo_rust(tmp_path):
    """Test que el resumidor genérico captura imports y firmas de Rust."""
    archivo = tmp_path / "lib.rs"
    archivo.write_text("use std::collections::HashMap;\n\npub fn procesar(x: i32) -> i32 { x }\n", encoding="utf-8")
    resumen = resumir_archivo(archivo, max_tokens=400)

    assert any(i.startswith("use std::collections::HashMap") for i in resumen["imports"])
    assert any(f.startswith("pub fn procesar") for f in resumen["firmas"])


def test_resumir_archivo_java(tmp_path):
    """Test que el resumidor genérico captura imports y firmas de Java."""
    archivo = tmp_path / "Main.java"
    archivo.write_text(
    ...
    assert any(i.startswith("import java.util.List") for i in resumen["imports"])
    assert any(f.startswith("public class Main") for f in resumen["firmas"])
    assert any(f.startswith("public static void main") for f in resumen["firmas"])


def test_resumir_archivo_extension_desconocida(tmp_path):
    """Test que una extensión desconocida usa el resumidor genérico sin imports ni firmas."""
    archivo = tmp_path / "datos.xyz"
    archivo.write_text("linea uno\nlinea dos\nlinea tres\n", encoding="utf-8")
    resumen = resumir_archivo(archivo, max_tokens=400)

    assert "resumen" in resumen
    assert "linea uno" in resumen["resumen"]
    assert "imports" not in resumen
    assert "firmas" not in resumen
